...
def test_llm_client_factory_singleton() -> None:
    ...
    with (
        ...
        mock_instructor.side_effect = [MagicMock(name="client1"), MagicMock(name="client3")]
        ...
        # Should only create OpenAI/instructor once
        ...
        num_call = 2
        assert mock_openai.call_count == num_call
...
def test_llm_client_factory_clear_cache() -> None:
    ...
    with (
        ...
        mock_instructor.return_value = "mock-client"
        ...
        LLMClientFactory.create()
        num_call = 2
        assert mock_openai.call_count == num_call  # Re-created after clear
...
def test_llm_client_factory_env_loaded_once() -> None:
    ...
    with (
        patch.dict(os.environ, {"OPENROUTER_API_KEY": "test-key"}),
        patch("jitsu.core.client.OpenAI"),
        patch("jitsu.core.client.instructor.from_openai") as mock_instructor,
        patch("jitsu.core.client.dotenv.load_dotenv") as mock_load,
    ):
        mock_instructor.return_value = MagicMock(name="env-client")
        ...
        LLMClientFactory.create()
        num_call = 2
        assert mock_load.call_count == num_call
...
def test_create_double_check_locking_coverage() -> None:
    ...
    class RacyLock:
        def __enter__(self) -> object:
            ...
            LLMClientFactory._instance_cache[LLMClientFactory.DEFAULT_BASE_URL] = "race-winner"
